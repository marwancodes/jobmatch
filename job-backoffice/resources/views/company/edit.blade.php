            ...
            <form action="{{ $formAction }}" method="POST">
                ...
                <div class="p-6 mb-4 border border-gray-100 rounded-lg shadow-sm bg-gray-50">
                    ...
                    <div class="mb-4">
                        ...
                        <select name="industry" id="industry" vlue="{{ old('industry', $company->industry)}}" class="{{ $errors->has('industry') ? 'outline-red-500 outline outline-1' : '' }} w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">
                ...
                <div class="p-6 mb-4 border border-gray-100 rounded-lg shadow-sm bg-gray-50">
                    ...
                    <div class="mb-4">
                        ...
                        <input disabled type="email" name="owner_email" id="owner_email" value="{{ old('owner_email', $company->owner->email)}}"
                            class="{{ $errors->has('owner_email') ? 'outline-red-500 outline outline-1' : '' }} w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 bg-gray-200">
                        @error('owner_email')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="mb-4">
                        <label for="owner_password" class="block mb-2 text-sm font-medium text-gray-700">Change Owner Password (Leave blank to keep the same)</label>
                        <div class="relative" x-data="{ showPassword: false}">
                            <x-text-input id="owner_password" class="block w-full mt-1" value="{{ old('owner_password')}}"
                                            name="owner_password"
                ...
                <div class="flex items-center justify-end">
                    @if (auth()->user()->role == 'company-owner')
                        <a href="{{ route('my-company.show') }}" class="px-4 mr-4 text-gray-500 rounded-md hover:text-gray-700">
                            Cancel
                        </a>
                    @endif
